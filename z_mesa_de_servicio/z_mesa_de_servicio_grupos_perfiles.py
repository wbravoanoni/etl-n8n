...
logging.basicConfig(level=logging.INFO, 
                    ...
                    handlers=[logging.FileHandler("logs/z_mesa_de_servicio_grupos_perfiles.log"),
...
try:
    ...
    for row in rows:
        ...
        tipo = '' if row[8] is None else str(row[8])
        fechaInicio = '' if row[9] is None else str(row[9])
        sSUSR_DateLastLogin = '' if row[10] is None else str(row[10])
        sSUSR_Initials = '' if row[11] is None else str(row[11])
        sSUSR_DateTo = '' if row[12] is None else str(row[12])
    ...
    try:
        ...
        conn_mysql.commit()
        logging.info("Tabla 'z_mesa_de_servicio_grupos_perfiles' truncada exitosamente.")
