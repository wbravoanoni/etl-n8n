...
EMAIL_ALERT = os.getenv("RECIPIENT_EMAILS_TEAMCODER_ERROR", EMAIL_USER)
...
# ========================
ARCHIVO_REPORTE = "z_teamcoder/resultados/z_teamcoder_descargaReporte_homologado.txt"
FECHA_REPORTE = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
NOMBRE_ADJUNTO = f"z_teamcoder_descargaReporte_homologado_{FECHA_REPORTE}.txt"

# ========================
# CONFIGURACIÓN LOG
# ========================
LOG_FILE_PATH = "z_teamcoder/logs/homologacion.log"
...
def notificar_fallo(error_mensaje):
    try:
        ...
        msg["From"] = EMAIL_USER
        msg["To"] = EMAIL_ALERT
        msg["Subject"] = f"ERROR en envío de Reporte Teamcoder - {FECHA_REPORTE}"

        cuerpo = f"""Hola,
            Se produjo un problema al enviar el reporte homologado de Teamcoder.
...
def enviar_correo(archivo, intentos=5, espera=10):
    ...
    for intento in range(1, intentos + 1):
        try:
            ...
            msg["From"] = EMAIL_USER
            msg["To"] = EMAIL_TO
            msg["Subject"] = f"Reporte Homologado - Teamcoder - {FECHA_REPORTE}"

            cuerpo = f"""
                Estimados, se adjunta registrso de pacientes por fecha de egreso en TrakCare generado automáticamente por el sistema.
                Fecha de egreso: {FECHA_REPORTE}
