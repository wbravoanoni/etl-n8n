...
try:
    ...
    query = ''' 
        SELECT
            ...
            (SELECT TOP 1
                ...
                FROM RB_APPOINTMENT 
                WHERE APPT_AS_ParRef->AS_Date <= PAADM_DischgDate  
                AND APPT_PAPMI_DR = PAADM_PAPMI_DR
                AND APPT_Adm_DR = PAADM_RowID 
                ORDER BY APPT_AS_ParRef->AS_Date DESC) AS "local",
            (SELECT TOP 1
                ...
                AND APPT_PAPMI_DR = PAADM_PAPMI_DR
                AND APPT_Adm_DR = PAADM_RowID 
                ORDER BY APPT_AS_ParRef->AS_Date DESC) AS "cod_recurso_cita",
            (SELECT TOP 1
                ...
                AND APPT_PAPMI_DR = PAADM_PAPMI_DR
                AND APPT_Adm_DR = PAADM_RowID 
                ORDER BY APPT_AS_ParRef->AS_Date DESC) AS "recurso_cita",
            (SELECT TOP 1 CONVERT(varchar,APPT_AS_ParRef->AS_Date,105)
                FROM RB_APPOINTMENT 
                WHERE APPT_AS_ParRef->AS_Date <= PAADM_DischgDate  
                AND APPT_PAPMI_DR = PAADM_PAPMI_DR    
                AND APPT_Adm_DR = PAADM_RowID 
                ORDER BY APPT_AS_ParRef->AS_Date DESC) AS "fecha_cita",
            (SELECT TOP 1
                (CASE
                ...
                AND APPT_PAPMI_DR = PAADM_PAPMI_DR    
                AND APPT_Adm_DR = PAADM_RowID 
                ORDER BY APPT_AS_ParRef->AS_Date DESC) AS "estado_cita",
    ...
    insert_query = """
        INSERT INTO z_usabilidad_coloproctologia_contrarreferencias (
            nro_episodio, nro_registro, local, cod_recurso_cita, recurso_cita,
            fecha_cita, estado_cita, fecha_contrarref, etapa_contrarreferencia,
            etapa_alta_amb, destino_alta, nro_lista_espera, establecimiento_origen,
            establecimiento_destino, diagnostico_cr, diagnostico, etapa_ges,
            tipo_diagnostico, diagnostico_principal, tratamiento_recibido,
            Indicaciones_al_alta, nombre_profesional_regitra, rut_profesional_registra,
            especialidad_le, referido_por_el_establecimiento_le,
            referido_por_el_establecimiento_contrarreferencia,
            referido_al_establecimiento_contrarreferencia, referido_por_alta_amb,
            cod_usuario_actualiza, usuario_actualiza, fechaActualizacion
        ) VALUES (
    ...
    try:
        ...
        for i in range(0, len(formatted_rows), chunk_size):
            cursor_mysql.executemany(insert_query, formatted_rows[i:i+chunk_size])
            conn_mysql.commit()
    except Exception as e:
