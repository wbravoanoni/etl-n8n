...
logging.basicConfig(level=logging.INFO,
    ...
    handlers=[
        logging.FileHandler("logs/z_teamcoder_descargaReporte_quimioterapia_ambulatoria.log"),
...
archivo_excel = f"z_teamcoder_quimioterapia_ambulatoria/entrada/z_teamcoder_descargaReporte_original.xlsx"
...
try:
    ...
    query = """
                ...
                '99' as OCUPACION,
                '99' AS CAT_OCP,
                ICD.MRCID_Code AS "CODIGO_CIE10",
                ICD.MRCID_Desc AS "DESC_CIE10",
                sched.AS_Date AS "FechaCita",
                sched.AS_SessStartTime AS "HoraCita"
                        FROM PA_ADM a
                INNER JOIN RB_Appointment appt
                    ON appt.APPT_Adm_DR = a.PAADM_RowId
                INNER JOIN RB_ApptSchedule sched
                    ON appt.APPT_AS_ParRef = sched.AS_RowId
                        ...
                        LEFT JOIN MR_ADM mradm ON mradm.MRADM_ADM_DR = a.PAADM_RowID
                        LEFT JOIN MR_Diagnos diag ON diag.MRDIA_MRADM_ParRef = mradm.MRADM_RowID
                        LEFT JOIN MRC_ICDDx ICD ON diag.MRDIA_ICDCode_DR = ICD.MRCID_RowID
                        WHERE 
                            a.PAADM_DepCode_DR->CTLOC_Hospital_DR = 10448
                            AND PAADM_DischgDate BETWEEN '2025-01-01' and '2025-01-31'
                AND a.PAADM_DepCode_DR->CTLOC_RowID = '4621' 
                -- AND a.PAADM_ADMNo='A0004983136' 
                AND appt.APPT_Status = 'D';
    """

# 4621	HDS-UQA	Unidad de Quimioterapia Ambulatoria
