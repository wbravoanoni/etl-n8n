...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/z_usabilidad_monitoreo_enfermeria_braden_UPC_UHI.log"),
...
def crear_tabla_z_cuestionario_braden(cursor_mysql):
    create_table_sql = """
    CREATE TABLE IF NOT EXISTS z_usabilidad_monitoreo_enfermeria_braden_UPC_UHI (
        ID VARCHAR(6),
...
try:
    ...
    query = """
        ...
        WHERE
            -- c.PAADM_ADMDATE >=  CAST(DATEADD(MONTH, -12, GETDATE()) AS DATE)  
            -- AND QUESCreateDate >= DATEADD(MONTH, -6, GETDATE())
            c.PAADM_ADMDATE >=  '2025-07-01'
            AND QUESCreateDate >= '2026-01-07' 
    ...
    conn_mysql.commit()
    logging.info("Tabla z_usabilidad_monitoreo_enfermeria_braden_UPC_UHI verificada/creada.")

    # Truncar tabla
    cursor_mysql.execute("TRUNCATE TABLE z_usabilidad_monitoreo_enfermeria_braden_UPC_UHI")
    conn_mysql.commit()
    ...
    # =========================
    insert_query = """
        INSERT INTO z_usabilidad_monitoreo_enfermeria_braden_UPC_UHI VALUES (
            %s, %s, %s, %s, %s, %s, %s, %s,
