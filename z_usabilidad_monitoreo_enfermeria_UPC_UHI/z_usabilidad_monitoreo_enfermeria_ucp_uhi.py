...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/z_usabilidad_monitoreo_enfermeria_ucp_uhi.log"),
...
try:
    ...
    SELECT DISTINCT 
        ...
        b.NOT_ParRef->MRADM_ADM_DR->PAADM_PAPMI_DR->PAPMI_No as "rut", 
		b.NOT_ParRef->MRADM_ADM_DR->PAADM_PAPMI_DR->PAPMI_Name2 as "nombre_paciente",
		b.NOT_ParRef->MRADM_ADM_DR->PAADM_PAPMI_DR->PAPMI_Name as "apellido_paciente",
        -- b.NOT_ParRef->MRADM_ADM_DR->PAADM_CurrentWard_DR,
		b.NOT_ParRef->MRADM_ADM_DR->PAADM_CurrentWard_DR->WARD_Desc AS local_actual
    FROM 
        MR_NursingNotes b
    JOIN 
        MR_Encounter a ON b.NOT_ParRef = a.ENC_MRAdm_DR
    WHERE 
        b.NOT_Date >= '2025-07-01'
        AND b.NOT_Hospital_DR = 10448
        AND a.ENC_StartDate >= '2026-01-07' 
		-- Enfermeros/as
        AND b.NOT_NurseId_DR->CTPCP_CarPrvTp_DR=57 
        AND b.NOT_ParRef->MRADM_ADM_DR->PAADM_CurrentWard_DR IN (416,402,417,509,428,415) 
    '''
    ...
    # TRUNCATE
    try:
        cursor_mysql.execute("TRUNCATE TABLE z_usabilidad_monitoreo_enfermeria_ucp_uhi")
        conn_mysql.commit()
    except Exception as e:
    ...
    # INSERT
    insert_query = """
        INSERT INTO z_usabilidad_monitoreo_enfermeria_ucp_uhi (
            NumeroEpisodio,
            Estado_Evolucion,
            Grupo_Evolucion,
            Tipo_Evolucion,
